...
def all(args):
    ...
    assembly_output = newbler_assembly.run_newbler(args.cpu, assembly_seq_cut_path, Output, args.minidentity, args.minoverlaplen)
...
def get_all_connection_from_datafile(data_list):
    '''
    Reading the Newbler assembly results file;
...
def add_seq(seeds, file_data_fna_name, simple_pairs, id_seq, id_length):
    ...
    with open(file_data_fna_name, 'r') as rfna:
        fna_contend = []    #Set a list that stores contig in PMATAllcontig.fna generated by Newbler
...
if __name__ == '__main__':
    ...
    all_sub = subparsers.add_parser('all', description=all_description + 
                                         all_example, formatter_class=argparse.RawTextHelpFormatter)

    optional_group=all_sub._action_groups.pop()
    required_group = all_sub.add_argument_group('Required arguments')
    ...
    all_sub._action_groups.append(optional_group)
    all_sub.set_defaults(func=all)
